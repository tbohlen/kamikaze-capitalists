...
public class Game {
    ...
    public final Building[][] buildings; // first index is x, second is y, e.g.
					 // buildings[x][y]
    ...
    private boolean connectedToCapital(int x, int y, Player player,
	    boolean[][] searched) {
	if (searched[x][y]) {
	    return false;
	}
	...
	if (b == null || !b.owner.equals(player)) {
	    ...
	    return connectedToCapital(x - 1, y, player, searched)
		    || connectedToCapital(x + 1, y, player, searched)
		    || connectedToCapital(x, y - 1, player, searched)
		    || connectedToCapital(x, y + 1, player, searched);
	}
    }
